...
import type { SheetStore, FactionStore, PlayerStore } from "../src/plugins/circles/router.ts";
...
function makeFactionStore(
  ...
  return {
    ...
    queryOne: (q) => Promise.resolve(
      records.find((r) =>
        Object.entries(q).every(([k, v]) => (r as Record<string, unknown>)[k] === v)
      ) ?? null,
    ),
    create: (e) => { records.push({ ...e }); return Promise.resolve(e); },
    modify: (q, _op, update) => {
...
function makePlayerStore(staff: boolean): PlayerStore {
  return { queryOne: () => Promise.resolve({ flags: staff ? "admin" : "player" }) };
}
...
function approvedSheet(overrides: Partial<ICharSheet> = {}): Partial<ICharSheet> {
  return {
    id: "p1", playerId: "p1", playbookId: "wolf", status: "approved",
...
function seedFaction(overrides: Partial<IFactionEntry> = {}): IFactionEntry {
  return {
    id: "f1", playerId: "p1", circle: "night",
    name: "The Undying Court", status: 1, notes: "",
...
Deno.test("all routes: no userId → 401", async () => {
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), makeFactionStore(), NORMAL);
  const routes = [
...
Deno.test("GET /api/v1/circles: returns status, ratings, and factions", async () => {
  ...
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), fac, NORMAL);
...
Deno.test("GET /api/v1/circles: no sheet → 404", async () => {
  const handler = makeCirclesRouter(makeSheetStore(null), makeFactionStore(), NORMAL);
  const res = await handler(req("GET", "/api/v1/circles"), "p1");
...
Deno.test("mark: decrements circle status by 1", async () => {
  const store = makeSheetStore(approvedSheet({ circleStatus: { mortalis: 0, night: 2, power: -1, wild: 1 } }));
  const handler = makeCirclesRouter(store, makeFactionStore(), NORMAL);
  const res = await handler(req("POST", "/api/v1/circles/mark", { circle: "night" }), "p1");
  assertEquals(res.status, 200);
  const body = await json(res);
...
Deno.test("mark: 409 when circle already at minimum", async () => {
  const handler = makeCirclesRouter(
    makeSheetStore(approvedSheet({ circleStatus: { mortalis: -3, night: 0, power: 0, wild: 0 } })),
    makeFactionStore(), NORMAL,
  );
  const res = await handler(req("POST", "/api/v1/circles/mark", { circle: "mortalis" }), "p1");
  assertEquals(res.status, 409);
});

Deno.test("mark: invalid circle name → 400", async () => {
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), makeFactionStore(), NORMAL);
  const res = await handler(req("POST", "/api/v1/circles/mark", { circle: "fire" }), "p1");
...
Deno.test("mark: 403 if sheet not approved", async () => {
  const handler = makeCirclesRouter(
    makeSheetStore(approvedSheet({ status: "draft" })), makeFactionStore(), NORMAL,
  );
  const res = await handler(req("POST", "/api/v1/circles/mark", { circle: "night" }), "p1");
...
Deno.test("mark: all four circles are valid", async () => {
  for (const circle of ["mortalis", "night", "power", "wild"]) {
    const store = makeSheetStore(approvedSheet({ circleStatus: { mortalis: 0, night: 0, power: 0, wild: 0 } }));
    const handler = makeCirclesRouter(store, makeFactionStore(), NORMAL);
    const res = await handler(req("POST", "/api/v1/circles/mark", { circle }), "p1");
...
Deno.test("improve: increments circle status by 1", async () => {
  const store = makeSheetStore(approvedSheet({ circleStatus: { mortalis: 0, night: 0, power: 0, wild: 1 } }));
  const handler = makeCirclesRouter(store, makeFactionStore(), NORMAL);
  const res = await handler(req("POST", "/api/v1/circles/improve", { circle: "wild" }), "p1");
  assertEquals(res.status, 200);
  const body = await json(res);
...
Deno.test("improve: 409 when circle already at maximum", async () => {
  ...
  const res = await handler(req("POST", "/api/v1/circles/improve", { circle: "mortalis" }), "p1");
  assertEquals(res.status, 409);
});

Deno.test("improve: invalid circle name → 400", async () => {
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), makeFactionStore(), NORMAL);
  const res = await handler(req("POST", "/api/v1/circles/improve", { circle: "shadow" }), "p1");
  assertEquals(res.status, 400);
});
...
Deno.test("adjust: positive delta increases status", async () => {
  const store = makeSheetStore(approvedSheet({ circleStatus: { mortalis: 0, night: -1, power: 0, wild: 0 } }));
  const handler = makeCirclesRouter(store, makeFactionStore(), NORMAL);
  const res = await handler(req("POST", "/api/v1/circles/adjust", { circle: "night", delta: 2 }), "p1");
  assertEquals(res.status, 200);
...
Deno.test("adjust: negative delta decreases status", async () => {
  const store = makeSheetStore(approvedSheet({ circleStatus: { mortalis: 2, night: 0, power: 0, wild: 0 } }));
  const handler = makeCirclesRouter(store, makeFactionStore(), NORMAL);
  await handler(req("POST", "/api/v1/circles/adjust", { circle: "mortalis", delta: -3 }), "p1");
  assertEquals((store._sheet!.circleStatus as Record<string, number>).mortalis, -1);
});

Deno.test("adjust: clamps to bounds", async () => {
  const store = makeSheetStore(approvedSheet({ circleStatus: { mortalis: 2, night: 0, power: 0, wild: 0 } }));
  const handler = makeCirclesRouter(store, makeFactionStore(), NORMAL);
  await handler(req("POST", "/api/v1/circles/adjust", { circle: "mortalis", delta: 99 }), "p1");
  assertEquals((store._sheet!.circleStatus as Record<string, number>).mortalis, 3);
});

Deno.test("adjust: missing delta → 400", async () => {
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), makeFactionStore(), NORMAL);
  const res = await handler(req("POST", "/api/v1/circles/adjust", { circle: "night" }), "p1");
  assertEquals(res.status, 400);
});

Deno.test("adjust: string delta → 400", async () => {
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), makeFactionStore(), NORMAL);
  const res = await handler(req("POST", "/api/v1/circles/adjust", { circle: "night", delta: "big" }), "p1");
  assertEquals(res.status, 400);
});
...
Deno.test("POST factions: creates faction entry", async () => {
  const fac = makeFactionStore();
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), fac, NORMAL);
  const res = await handler(req("POST", "/api/v1/circles/factions", {
    circle: "night", name: "The Undying Court", status: 1, notes: "They owe me",
  }), "p1");
...
Deno.test("POST factions: status defaults to 0 if omitted", async () => {
  ...
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), fac, NORMAL);
  const res = await handler(req("POST", "/api/v1/circles/factions", {
    circle: "mortalis", name: "City Hall",
  }), "p1");
  assertEquals(res.status, 201);
...
Deno.test("POST factions: invalid circle → 400", async () => {
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), makeFactionStore(), NORMAL);
  const res = await handler(req("POST", "/api/v1/circles/factions", { circle: "chaos", name: "X" }), "p1");
  assertEquals(res.status, 400);
});

Deno.test("POST factions: missing name → 400", async () => {
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), makeFactionStore(), NORMAL);
  const res = await handler(req("POST", "/api/v1/circles/factions", { circle: "night" }), "p1");
  assertEquals(res.status, 400);
});

Deno.test("POST factions: blank name → 400", async () => {
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), makeFactionStore(), NORMAL);
  const res = await handler(req("POST", "/api/v1/circles/factions", { circle: "night", name: "  " }), "p1");
  assertEquals(res.status, 400);
});
...
Deno.test("PATCH factions/:id: updates name", async () => {
  const fac = makeFactionStore([seedFaction()]);
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), fac, NORMAL);
  const res = await handler(req("PATCH", "/api/v1/circles/factions/f1", { name: "The Dusk Parliament" }), "p1");
  assertEquals(res.status, 200);
...
Deno.test("PATCH factions/:id: updates status", async () => {
  const fac = makeFactionStore([seedFaction()]);
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), fac, NORMAL);
  const res = await handler(req("PATCH", "/api/v1/circles/factions/f1", { status: -2 }), "p1");
  assertEquals(res.status, 200);
...
Deno.test("PATCH factions/:id: updates notes", async () => {
  const fac = makeFactionStore([seedFaction()]);
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), fac, NORMAL);
  await handler(req("PATCH", "/api/v1/circles/factions/f1", { notes: "New notes" }), "p1");
...
Deno.test("PATCH factions/:id: no valid fields → 400", async () => {
  const fac = makeFactionStore([seedFaction()]);
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), fac, NORMAL);
  const res = await handler(req("PATCH", "/api/v1/circles/factions/f1", { color: "red" }), "p1");
  assertEquals(res.status, 400);
});
...
Deno.test("PATCH factions/:id: not owner → 403", async () => {
  ...
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), fac, NORMAL);
  const res = await handler(req("PATCH", "/api/v1/circles/factions/f1", { name: "X" }), "p1");
  assertEquals(res.status, 403);
});

Deno.test("PATCH factions/:id: staff can edit any", async () => {
  const fac = makeFactionStore([seedFaction({ playerId: "p2" })]);
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), fac, STAFF);
  const res = await handler(req("PATCH", "/api/v1/circles/factions/f1", { name: "X" }), "staff1");
  assertEquals(res.status, 200);
});

Deno.test("PATCH factions/:id: not found → 404", async () => {
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), makeFactionStore(), NORMAL);
  const res = await handler(req("PATCH", "/api/v1/circles/factions/no-id", { name: "X" }), "p1");
  assertEquals(res.status, 404);
});
...
Deno.test("DELETE factions/:id: staff removes any", async () => {
  const fac = makeFactionStore([seedFaction({ playerId: "p2" })]);
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), fac, STAFF);
  const res = await handler(req("DELETE", "/api/v1/circles/factions/f1"), "staff1");
  assertEquals(res.status, 200);
  assertEquals(fac._records.length, 0);
});
...
Deno.test("DELETE factions/:id: not found → 404", async () => {
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), makeFactionStore(), NORMAL);
  const res = await handler(req("DELETE", "/api/v1/circles/factions/no-id"), "p1");
  assertEquals(res.status, 404);
});
...
Deno.test("GET circles/:id: staff views any", async () => {
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet({ id: "p2" })), makeFactionStore(), STAFF);
  const res = await handler(req("GET", "/api/v1/circles/p2"), "staff1");
  assertEquals(res.status, 200);
});

Deno.test("GET circles/:id: non-staff views other → 403", async () => {
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet({ id: "p2" })), makeFactionStore(), NORMAL);
  const res = await handler(req("GET", "/api/v1/circles/p2"), "p1");
  assertEquals(res.status, 403);
});

Deno.test("GET circles/:id: not found → 404", async () => {
  const handler = makeCirclesRouter(makeSheetStore(null), makeFactionStore(), STAFF);
...
Deno.test("PATCH circles/:id: staff sets all circle statuses", async () => {
  ...
  const res = await handler(req("PATCH", "/api/v1/circles/p2", {
...
Deno.test("PATCH circles/:id: staff values are clamped", async () => {
  const store = makeSheetStore(approvedSheet({ id: "p2" }));
  const handler = makeCirclesRouter(store, makeFactionStore(), STAFF);
  await handler(req("PATCH", "/api/v1/circles/p2", {
...
Deno.test("PATCH circles/:id: no valid fields → 400", async () => {
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet({ id: "p2" })), makeFactionStore(), STAFF);
  const res = await handler(req("PATCH", "/api/v1/circles/p2", { color: "red" }), "staff1");
  assertEquals(res.status, 400);
});
...
Deno.test("DELETE /api/v1/circles → 404", async () => {
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), makeFactionStore(), NORMAL);
  assertEquals((await handler(req("DELETE", "/api/v1/circles"), "p1")).status, 404);
});

Deno.test("GET /api/v1/circles/id/extra → 404", async () => {
  const handler = makeCirclesRouter(makeSheetStore(approvedSheet()), makeFactionStore(), NORMAL);
  assertEquals((await handler(req("GET", "/api/v1/circles/p1/extra"), "p1")).status, 404);
});
