...
import { markXP, canTakeAdvance, validateAdvance, applyAdvance, XP_PER_ADVANCE } from "./logic.ts";
...
async function fileMajorAdvanceJob(sheet: ICharSheet, adv: IPlaybookAdvance): Promise<IJob> {
  const pb = getPlaybook(sheet.playbookId);
  const number = await getNextJobNumber();
  const descriptions: Record<string, string> = {
    "retire":          "This character is retiring to safety. Work with the player to close out their story.",
    "change-playbook": "This character is changing playbooks. Coordinate the transition with the player.",
    "move-any":        "This character is taking a move from any playbook. Confirm the choice is narratively sound.",
  };
  const now = Date.now();
  const job: IJob = {
    id: `job-${number}`,
    ...
    description: [
      `**${sheet.name}** (${pb?.name ?? sheet.playbookId}) has taken a major advance: **${adv.label}**`,
      ``,
...
function xpBar(xp: number, max = XP_PER_ADVANCE): string {
  return Array.from({ length: max }, (_, i) => i < xp ? "%ch%cg(*)%cn" : "( )").join("");
}
...
addCmd({
  ...
  exec: async (u) => {
    ...
    u.send(`%ch+xp/mark:%cn  ${who} XP: ${xpBar(result.xp)}  (${result.xp}/${XP_PER_ADVANCE})${result.xp >= XP_PER_ADVANCE ? " — %ch%cgReady to advance!%cn" : ""}`);
  },
});
...
addCmd({
  ...
  exec: async (u) => {
    ...
    const pb = getPlaybook(sheet.playbookId);
    if (!pb) { u.send("%ch+advance:%cn  Unknown playbook."); return; }
    ...
    for (const adv of pb.advances) {
      ...
      const maxLabel = adv.maxTimes && adv.maxTimes > 1 ? ` (${taken}/${adv.maxTimes})` : "";
      const majorTag = adv.major ? " %ch%cy[major]%cn" : "";

      if (err) {
        const reason = err === "not-enough-xp" ? "not enough XP"
          : err === "major-advance-locked" ? "need 5 regular advances first"
          : err === "advance-maxed" ? "maxed out"
          : err === "stat-at-max" ? "stat already at max"
          : err;
        lines.push(`  %cx${adv.id}${maxLabel}%cn  ${adv.label}${majorTag}  %ch%cx(${reason})%cn`);
      } else {
...
addCmd({
  ...
  exec: async (u) => {
    ...
    const pb = getPlaybook(sheet.playbookId);
    if (!pb) { u.send("%ch+advance/take:%cn  Unknown playbook."); return; }

    const error = validateAdvance(sheet, advanceId, pb);
    if (error) {
      const msg: Record<string, string> = {
        "not-enough-xp":        `You need ${XP_PER_ADVANCE} XP to advance (have ${sheet.xp}).`,
        "unknown-advance":       `Unknown advance: '${advanceId}'. Use +advance to see options.`,
        "major-advance-locked":  "Major advances require 5 regular advances first.",
        "advance-maxed":         "You've already taken that advance the maximum number of times.",
    ...
    const result = await sheets.atomicModify(u.me.id, (current: ICharSheet) => {
      ...
      return { ...current, xp: applied.xp, takenAdvances: applied.takenAdvances, stats: applied.stats, updatedAt: Date.now() };
    });
    ...
    if (adv.major) {
      ...
      u.send(`%ch+advance/take:%cn  %cy%chThis is a major advance — staff has been notified (Job #${job.number}).%cn`);
    }
  },
});
...
addCmd({
  ...
  exec: async (u) => {
    if (!isStaff(u)) { u.send("%ch+advance/xp:%cn  Staff only."); return; }

    const targetName = (u.cmd.args[0] ?? "").trim();
    const amount = Math.min(XP_PER_ADVANCE, Math.max(0, parseInt(u.cmd.args[1] ?? "0", 10)));

    const target = await sheets.queryOne({ name: targetName } as Parameters<typeof sheets.queryOne>[0]);
    if (!target) { u.send(`%ch+advance/xp:%cn  No sheet for '${targetName}'.`); return; }
