...
import { jobs, getNextJobNumber, jobHooks } from "ursamu/jobs";
...
async function fileIncapacitatedJob(sheet: ICharSheet): Promise<IJob> {
  ...
  const job: IJob = {
    ...
    description: [
      `**${sheet.name}** (${pb?.name ?? sheet.playbookId}) has taken their 5th harm and is now incapacitated.`,
      ``,
...
async function fileCorruptionAdvanceJob(sheet: ICharSheet, advanceName: string): Promise<IJob> {
  const pb = getPlaybook(sheet.playbookId);
  const number = await getNextJobNumber();
  const now = Date.now();
  const job: IJob = {
    ...
    submitterName: sheet.name,
    description: [
      `**${sheet.name}** (${pb?.name ?? sheet.playbookId}) filled their corruption track and took a corruption advance: **${advanceName}**`,
      ``,
...
function corruptionDots(marks: number, max = 5): string {
  return Array.from({ length: max }, (_, i) => i < marks ? "%ch%cy(*)%cn" : "( )").join("");
}
...
function harmDisplay(sheet: ICharSheet): string {
  ...
  const lines = [
    ...
    `%ch--- Corruption -------------------------------------%cn`,
    `  Marks:  ${corruptionDots(sheet.corruption.marks)}  (${sheet.corruption.marks}/5)`,
  ];

  if (sheet.corruption.advances.length) {
    lines.push(`  Advances: ${sheet.corruption.advances.map((a) => `%ch${a}%cn`).join(", ")}`);
  }

  return lines.join("\n");
}

async function getApprovedSheet(u: { me: { id: string }; send(msg: string): void }): Promise<ICharSheet | null> {
...
addCmd({
  ...
  exec: async (u) => {
    ...
    if (targetName) {
      const player = await sheets.queryOne({ name: targetName } as Parameters<typeof sheets.queryOne>[0]);
      if (!player) {
    ...
    if (!sheet) { u.send("%ch+harm/mark:%cn  No approved sheet found."); return; }
    if (sheet.status !== "approved") { u.send("%ch+harm/mark:%cn  Sheet must be approved."); return; }
    ...
    if (noChange) {
      u.send("%ch+harm/mark:%cn  All harm boxes are already marked (incapacitated).");
      return;
    }

    const count = markedHarmCount(result.harm);
    const incap = isIncapacitated(result.harm) ? " %ch%cr— INCAPACITATED!%cn" : "";
    u.send(`%ch+harm/mark:%cn  Harm marked. ${harmBar(result.harm.boxes)}  (${count}/5)${incap}`);

    if (isIncapacitated(result.harm)) {
      const job = await fileIncapacitatedJob(result);
      u.send(`%ch+harm/mark:%cn  %cr%chStaff notified of incapacitation (Job #${job.number}).%cn`);
    }
  },
});
...
addCmd({
  ...
  exec: async (u) => {
    ...
    const sheet = await sheets.queryOne({ id: targetId } as Parameters<typeof sheets.queryOne>[0]) as ICharSheet | null;
    if (!sheet || sheet.status !== "approved") { u.send("%ch+harm/heal:%cn  No approved sheet found."); return; }
    ...
    u.send(`%ch+harm/heal:%cn  Healed ${actualHealed} box(es). ${harmBar(result.harm.boxes)}  (${remaining}/5)`);
  },
});
...
addCmd({
  ...
  exec: async (u) => {
    ...
    if (targetName) {
      if (!isStaff(u)) { u.send("%ch+harm/armor:%cn  Only staff can set armor on others."); return; }
      const player = await sheets.queryOne({ name: targetName } as Parameters<typeof sheets.queryOne>[0]);
      if (!player) { u.send(`%ch+harm/armor:%cn  No sheet found for '${targetName}'.`); return; }
      targetId = (player as ICharSheet).id;
    }

    const sheet = await sheets.queryOne({ id: targetId } as Parameters<typeof sheets.queryOne>[0]) as ICharSheet | null;
    if (!sheet || sheet.status !== "approved") { u.send("%ch+harm/armor:%cn  No approved sheet found."); return; }
...
addCmd({
  ...
  exec: async (u) => {
    ...
    if (targetName) {
      if (!isStaff(u)) { u.send("%ch+corruption/mark:%cn  Only staff can mark corruption on others."); return; }
      const player = await sheets.queryOne({ name: targetName } as Parameters<typeof sheets.queryOne>[0]);
      if (!player) { u.send(`%ch+corruption/mark:%cn  No sheet for '${targetName}'.`); return; }
      targetId = (player as ICharSheet).id;
    }

    const sheet = await sheets.queryOne({ id: targetId } as Parameters<typeof sheets.queryOne>[0]) as ICharSheet | null;
    if (!sheet || sheet.status !== "approved") { u.send("%ch+corruption/mark:%cn  No approved sheet found."); return; }

    let advanceTriggered = false;
    const result = await sheets.atomicModify(targetId, (current: ICharSheet) => {
      const { corruption: updated, advanceTriggered: triggered } = markCorruption(current.corruption);
    ...
    if (advanceTriggered) {
      u.send(
        `%ch+corruption/mark:%cn  ${corruptionDots(result.corruption.marks)} — ` +
        `%ch%cr5 marks! Take a Corruption advance with +corruption/take <advance name>.%cn`
      );
    } else {
      u.send(`%ch+corruption/mark:%cn  ${corruptionDots(result.corruption.marks)}  (${result.corruption.marks}/5)`);
    }
  },
});
...
addCmd({
  ...
  exec: async (u) => {
    ...
    if (advanceName.length > 100) {
      u.send("%ch+corruption/take:%cn  Advance name cannot exceed 100 characters.");
      return;
    }

    const sheet = await sheets.queryOne({ id: u.me.id } as Parameters<typeof sheets.queryOne>[0]) as ICharSheet | null;
    if (!sheet || sheet.status !== "approved") { u.send("%ch+corruption/take:%cn  No approved sheet found."); return; }
    ...
    const result = await sheets.atomicModify(u.me.id, (current: ICharSheet) => {
      ...
      return { ...current, corruption: takeCorruptionAdvance(current.corruption, advanceName), updatedAt: Date.now() };
    });

    u.send(`%ch+corruption/take:%cn  Corruption advance recorded: %ch${advanceName}%cn. Marks reset.`);

    const job = await fileCorruptionAdvanceJob(result, advanceName);
    u.send(`%ch+corruption/take:%cn  %cy%chStaff notified (Job #${job.number}).%cn`);
  },
});
...
addCmd({
  ...
  exec: async (u) => {
    if (!isStaff(u)) { u.send("%ch+corruption/clear:%cn  Staff only."); return; }
    ...
    if (targetName) {
      const player = await sheets.queryOne({ name: targetName } as Parameters<typeof sheets.queryOne>[0]);
      if (!player) { u.send(`%ch+corruption/clear:%cn  No sheet for '${targetName}'.`); return; }
      targetId = (player as ICharSheet).id;
    }

    const sheet = await sheets.queryOne({ id: targetId } as Parameters<typeof sheets.queryOne>[0]) as ICharSheet | null;
    if (!sheet) { u.send("%ch+corruption/clear:%cn  No sheet found."); return; }
