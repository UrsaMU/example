...
function npcLine(n: INPC): string {
  ...
  return `  %ch${n.name}%cn${circle}  Harm: ${
    harmBar(n.harm.boxes)
  } (${count}/5)  Armor: ${n.harm.armor}${incap}`;
}
...
addCmd({
  ...
  exec: async (u) => {
    if (!isStaff(u)) {
      u.send("%ch+npc/create:%cn  Staff only.");
      return;
    }

    const name = (u.cmd.args[0] ?? "").trim();
    if (!name) {
      u.send("%ch+npc/create:%cn  Name cannot be blank.");
      return;
    }
    if (name.length > 100) {
      u.send("%ch+npc/create:%cn  Name cannot exceed 100 characters.");
      return;
    }
    ...
    u.send(
      `%ch+npc/create:%cn  Created NPC: %ch${name}%cn  [id: ${
        npc.id.slice(0, 8)
      }]`,
    );
  },
...
addCmd({
  ...
  exec: async (u) => {
    if (!isStaff(u)) {
      u.send("%ch+npc/list:%cn  Staff only.");
      return;
    }

    const all = await npcs.all();
    if (!all.length) {
      u.send("%ch+npc/list:%cn  No NPCs tracked yet. Use +npc/create <name>.");
      return;
    }
...
addCmd({
  ...
  exec: async (u) => {
    if (!isStaff(u)) {
      u.send("%ch+npc/view:%cn  Staff only.");
      return;
    }

    const npc = await findNPC((u.cmd.args[0] ?? "").trim());
    if (!npc) {
      u.send(`%ch+npc/view:%cn  No NPC found for '${u.cmd.args[0]}'.`);
      return;
    }
...
addCmd({
  ...
  exec: async (u) => {
    if (!isStaff(u)) {
      u.send("%ch+npc/harm:%cn  Staff only.");
      return;
    }

    const npc = await findNPC((u.cmd.args[0] ?? "").trim());
    if (!npc) {
      u.send(`%ch+npc/harm:%cn  No NPC found for '${u.cmd.args[0]}'.`);
      return;
    }
    ...
    const incap = isIncapacitated(result.harm)
      ...
      : "";
    u.send(
      `%ch+npc/harm:%cn  %ch${result.name}%cn  ${
...
addCmd({
  ...
  category: "Urban Shadows",
  help:
    "+npc/heal <name> [n]  —  [Staff] Heal n Harm boxes on an NPC (default 1).",
  pattern: /^\+npc\/heal\s+(.+?)(?:\s+(\d+))?$/i,
  exec: async (u) => {
    if (!isStaff(u)) {
      u.send("%ch+npc/heal:%cn  Staff only.");
      return;
    }
    ...
    const npc = await findNPC((u.cmd.args[0] ?? "").trim());
    if (!npc) {
      u.send(`%ch+npc/heal:%cn  No NPC found for '${u.cmd.args[0]}'.`);
      return;
    }
    ...
    u.send(
      `%ch+npc/heal:%cn  %ch${result.name}%cn  Healed ${actualHealed}. ${
        harmBar(result.harm.boxes)
      }  (${remaining}/5)`,
    );
  },
...
addCmd({
  ...
  exec: async (u) => {
    if (!isStaff(u)) {
      u.send("%ch+npc/armor:%cn  Staff only.");
      return;
    }
    ...
    const npc = await findNPC((u.cmd.args[0] ?? "").trim());
    if (!npc) {
      u.send(`%ch+npc/armor:%cn  No NPC found for '${u.cmd.args[0]}'.`);
      return;
    }
    ...
    u.send(
      `%ch+npc/armor:%cn  %ch${result.name}%cn  Armor set to %ch${result.harm.armor}%cn.`,
    );
  },
});
...
addCmd({
  ...
  category: "Urban Shadows",
  help:
    "+npc/circle <name>=<circle>  —  [Staff] Set an NPC's circle affiliation.",
  pattern: /^\+npc\/circle\s+(.+?)=(.+)$/i,
  exec: async (u) => {
    if (!isStaff(u)) {
      u.send("%ch+npc/circle:%cn  Staff only.");
      return;
    }

    const circle = (u.cmd.args[1] ?? "").trim().toLowerCase();
    if (circle.length > 50) {
      u.send("%ch+npc/circle:%cn  Circle name too long.");
      return;
    }

    const npc = await findNPC((u.cmd.args[0] ?? "").trim());
    if (!npc) {
      u.send(`%ch+npc/circle:%cn  No NPC found for '${u.cmd.args[0]}'.`);
      return;
    }
    ...
    u.send(
      `%ch+npc/circle:%cn  %ch${result.name}%cn  Circle set to %ch${
        result.circle ?? "(none)"
      }%cn.`,
    );
  },
...
addCmd({
  ...
  exec: async (u) => {
    if (!isStaff(u)) {
      u.send("%ch+npc/note:%cn  Staff only.");
      return;
    }

    const notes = (u.cmd.args[1] ?? "").trim();
    if (notes.length > 2000) {
      u.send("%ch+npc/note:%cn  Notes cannot exceed 2000 characters.");
      return;
    }

    const npc = await findNPC((u.cmd.args[0] ?? "").trim());
    if (!npc) {
      u.send(`%ch+npc/note:%cn  No NPC found for '${u.cmd.args[0]}'.`);
      return;
    }
...
addCmd({
  ...
  exec: async (u) => {
    if (!isStaff(u)) {
      u.send("%ch+npc/del:%cn  Staff only.");
      return;
    }

    const npc = await findNPC((u.cmd.args[0] ?? "").trim());
    if (!npc) {
      u.send(`%ch+npc/del:%cn  No NPC found for '${u.cmd.args[0]}'.`);
      return;
    }
