...
addCmd({
  ...
  exec: async (u) => {
    ...
    if (!sheet) {
      u.send(
        `%ch+who/view:%cn  No approved character found for '${u.cmd.args[0]}'.`,
      );
      return;
    }
    ...
    const incapFlag = isIncapacitated(sheet.harm)
      ? ` ${R}${H}[INCAPACITATED]${N}`
      : "";
    ...
    u.send([
      ...
      `  Circles:    mortalis ${sign(cs.mortalis)}  night ${
        sign(cs.night)
      }  power ${sign(cs.power)}  wild ${sign(cs.wild)}`,
    ].join("\n"));
  },
});
