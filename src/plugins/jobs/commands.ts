...
const H = "%ch"; const N = "%cn"; const G = "%cg"; const R = "%cr"; const Y = "%cy"; const DIM = "%cx";
...
addCmd({
  ...
  exec: async (u) => {
    ...
    if (!pool.length) {
      u.send(staff ? "%ch+jobs:%cn  No active jobs." : "%ch+jobs:%cn  No open jobs.");
      return;
    }

    const lines = [
      `${H}--- Jobs${staff ? " [all active]" : ""} (${pool.length}) ----------------------------${N}`,
    ];
...
addCmd({
  ...
  pattern: /^\+jobs\/all$/i,
  exec: async (u) => {
    if (!isStaff(u)) { u.send("%ch+jobs/all:%cn  Staff only."); return; }

    const all = (await jobs.all() as IJob[]).sort((a, b) => b.number - a.number);
    if (!all.length) { u.send("%ch+jobs/all:%cn  No jobs exist yet."); return; }

    const lines = [`${H}--- All Jobs (${all.length}) ----------------------------------${N}`];
...
addCmd({
  ...
  exec: async (u) => {
    ...
    if (!job) { u.send(`%ch+jobs/view:%cn  No job found for '${fragment}'.`); return; }
    ...
    if (visComments.length) {
      lines.push(`${H}--- Comments (${visComments.length}) ----------------------------${N}`);
...
addCmd({
  ...
  exec: async (u) => {
    ...
    if (!text) { u.send("%ch+jobs/comment:%cn  Comment cannot be blank."); return; }
    if (text.length > 2000) { u.send("%ch+jobs/comment:%cn  Comment cannot exceed 2000 characters."); return; }

    const job = await findJob(fragment, u.me.id, staff);
    if (!job) { u.send(`%ch+jobs/comment:%cn  No job found for '${fragment}'.`); return; }
    if (job.status === "closed" || job.status === "resolved") {
      u.send(`%ch+jobs/comment:%cn  Job #${job.number} is ${job.status} and cannot be commented on.`);
      return;
    }

    const comment = { id: crypto.randomUUID(), author: u.me.id, authorName: myName(u), text, staffOnly: false, createdAt: Date.now() };
...
addCmd({
  ...
  exec: async (u) => {
    if (!isStaff(u)) { u.send("%ch+jobs/approve:%cn  Staff only."); return; }

    const fragment = (u.cmd.args[0] ?? "").trim();
    const job = await findJob(fragment, u.me.id, true);
    if (!job) { u.send(`%ch+jobs/approve:%cn  No job found for '${fragment}'.`); return; }
    if (job.status === "resolved" || job.status === "closed") {
      u.send(`%ch+jobs/approve:%cn  Job #${job.number} is already ${job.status}.`);
      return;
    }
    ...
    u.send(`%ch+jobs/approve:%cn  ${G}${H}Resolved${N} job #${job.number}: "${job.title}"`);
  },
});
...
addCmd({
  ...
  exec: async (u) => {
    if (!isStaff(u)) { u.send("%ch+jobs/reject:%cn  Staff only."); return; }

    const fragment = (u.cmd.args[0] ?? "").trim();
    const reason = (u.cmd.args[1] ?? "").trim();

    if (!reason) { u.send("%ch+jobs/reject:%cn  Reason cannot be blank."); return; }
    if (reason.length > 2000) { u.send("%ch+jobs/reject:%cn  Reason cannot exceed 2000 characters."); return; }

    const job = await findJob(fragment, u.me.id, true);
    if (!job) { u.send(`%ch+jobs/reject:%cn  No job found for '${fragment}'.`); return; }
    if (job.status === "resolved" || job.status === "closed") {
      u.send(`%ch+jobs/reject:%cn  Job #${job.number} is already ${job.status}.`);
      return;
    }

    const rejComment = { id: crypto.randomUUID(), author: u.me.id, authorName: myName(u), text: reason, staffOnly: false, createdAt: Date.now() };
    const newComments = [...job.comments, rejComment as IJob["comments"][number]];
    const now = Date.now();
    await jobs.modify(
      { id: job.id } as Parameters<typeof jobs.modify>[0],
      "$set",
    ...
    const updated = { ...job, status: "closed" as const, comments: newComments, updatedAt: now };
    await jobHooks.emit("job:closed", updated);
    u.send(`%ch+jobs/reject:%cn  ${R}Closed${N} job #${job.number}: "${job.title}"\n  Reason: ${reason}`);
  },
});
...
addCmd({
  ...
  exec: async (u) => {
    if (!isStaff(u)) { u.send("%ch+jobs/close:%cn  Staff only."); return; }

    const fragment = (u.cmd.args[0] ?? "").trim();
    const job = await findJob(fragment, u.me.id, true);
    if (!job) { u.send(`%ch+jobs/close:%cn  No job found for '${fragment}'.`); return; }
    if (job.status === "resolved" || job.status === "closed") {
      u.send(`%ch+jobs/close:%cn  Job #${job.number} is already ${job.status}.`);
      return;
    }
    ...
    await jobHooks.emit("job:closed", updated);
    u.send(`%ch+jobs/close:%cn  ${DIM}Closed${N} job #${job.number}: "${job.title}"`);
  },
});
