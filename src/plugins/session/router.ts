...
export function makeSessionRouter(
  ...
  return async function sessionRouteHandler(
    ...
    if (path === "/api/v1/sessions" && method === "GET") {
      const all = (await sessDb.query() as ISession[]).sort((a, b) =>
        b.number - a.number
      );
    ...
    if (path === "/api/v1/sessions/current" && method === "GET") {
      const sess = (await getActiveSession(sessDb)) ??
        (await getEndedSession(sessDb));
    ...
    if (path === "/api/v1/sessions" && method === "POST") {
      ...
      if (existing) {
        return err(`Session #${existing.number} is already active`, 409);
      }
      ...
      const sess: ISession = {
        ...
        startedAt: now,
        startedBy: userId,
        startedByName: typeof body.startedByName === "string"
          ? body.startedByName
          : userId,
      };
    ...
    const myAnsMatch = path.match(
      /^\/api\/v1\/sessions\/([^/]+)\/answers\/my$/,
    );
    ...
    if (endMatch && method === "POST") {
      if (!staff) return err("Forbidden", 403);
      const sess = await sessDb.queryOne({ id: endMatch[1] }) as
        | ISession
        | null;
    ...
    if (myAnsMatch && method === "GET") {
      const sess = await sessDb.queryOne({ id: myAnsMatch[1] }) as
        | ISession
        | null;
      if (!sess) return err("Session not found", 404);

      const rec = await answerDb.queryOne({
        playerId: userId,
        sessionId: sess.id,
      });
    ...
    if (ansMatch && method === "POST") {
      ...
      if (sess.status !== "ended") {
        return err("Session is not in ended state", 409);
      }
      ...
      let rec = await answerDb.queryOne({
        playerId: userId,
        sessionId: sess.id,
      }) as ISessionAnswers | null;
      if (!rec) {
      ...
      for (const [qId, val] of Object.entries(incoming)) {
        ...
        if (val !== true && val !== false) {
          return err(`Answer for ${qId} must be true or false`);
        }
    ...
    if (submitMatch && method === "POST") {
      const sess = await sessDb.queryOne({ id: submitMatch[1] }) as
        | ISession
        | null;
      if (!sess) return err("Session not found", 404);
    ...
    if (sessMatch && method === "GET") {
      const sess = await sessDb.queryOne({ id: sessMatch[1] }) as
        | ISession
        | null;
      if (!sess) return err("Session not found", 404);
      return ok(sess);
